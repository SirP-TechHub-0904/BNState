...
namespace BNState.Pages.Shared.ViewComponents
{
    public class HomeSearchViewComponent : ViewComponent
    {
        ...
        public HomeSearchViewComponent(
            UserManager<BNState.Domain.Models.Profile> userManager, ApplicationDbContext context/*, IAmazonS3 s3Client*/, IStorageService storageService, IConfiguration config)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync(string key)
        {
            ...
            var s3Obj = new BNState.Domain.Dtos.AwsDtos.S3GetObjectDto()
            {
                Bucket = "lojourxyz",
                Key = key
            };
